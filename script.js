...
const listItems = ["MarkWeb", "Linkedin", "Instagram", "Facebook", "Twitter", "Youtube"];

const picTemplate = (address) => {
  return `
      <img src="${address}" height="21px" width="23px"></img>
  `;
};
...
dropdownList.addEventListener("click", (e) => {
  ...
  const clickedItemPic = pics[clickedItemText];

  if (clickedItemText in icons) {
    dropdownTitleIcon.innerHTML = iconTemplate(clickedItemIcon);
  } else {
    dropdownTitleIcon.innerHTML = picTemplate(clickedItemPic);
  }
...
dropdownList.addEventListener("mousemove", (e) => {
  ...
  const hoverItemTextP = pics[targetText];

  if (targetText in icons) {
    floatingIcon.innerHTML = iconTemplate(hoverItemText);
  } else {
    floatingIcon.innerHTML = picTemplate(hoverItemTextP);
  }
